...
class LossWrapper(torch.nn.Module):
    ...
    def forward(self, fc_feats, labels, masks, status, att_masks, gts, gt_indices, sc_flag):
        out = {}
        if not sc_flag:

            outputs, status_outputs = self.model(fc_feats, fc_feats, labels, status, att_masks)
            ...
            with torch.no_grad():
                greedy_seq, _, greedy_left_child, greedy_right_child, greedy_sibling, greedy_status, status_logprobs, _ \
                    = self.model(fc_feats, fc_feats, att_masks, mode='sample')
            ...
            gen_seq, sample_logprobs, gen_left_child, gen_right_child, gen_sibling, gen_status, gen_status_logprobs, status_outputs \
                = self.model(fc_feats, fc_feats, att_masks, opt={'sample_method': 'sample'}, mode='sample')
